...
class Kriteria extends CI_Controller
{
    ...
    function index()
    {
		...
		$this->load->view('element/admin_sidebar');
		$this->load->view('element/admin_topbar');
        $d['data']=$this->mod_kriteria->kriteria_data();
        $this->load->view('tu/master/kriteria/kriteriaview',$d);
    ...
    function add()
    {
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->mod_kriteria->kriteria_add($nama)==TRUE)
			{
				...
				redirect(base_url().'tu/master/kriteria');
			}else{
				set_header_message('danger','Tambah Kriteria','Gagal menambah kriteria');
				redirect(base_url().'tu/master/kriteria/add');
			}
		}else{
			$meta['judul']="Tambah Kriteria";
        	$this->load->view('tema/header',$meta);
        	$this->load->view('tu/master/kriteria/kriteriaadd');
	...
	function edit()
    {
		...
		if($this->form_validation->run()==TRUE)
		{
        	...
        	$this->load->view('tu/master/kriteria/kriteriaedit',$d);
        	$this->load->view('tema/footer');
		}
	}
	...
	function subkriteria()
	{
        ...
        $this->load->view('tu/master/kriteria/subkriteria/subkriteriaview',$d);
        $this->load->view('tema/footer');
	}
	...
	function subkriteriaadd()
	{
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			if($this->mod_kriteria->subkriteria_add($tipe,$kriteriaid,$opmax,$isi,$opmin,$min,$nilaiid)==TRUE)
			{
				...
				redirect(base_url('tu/master/kriteria/subkriteria').$link);
			}else{
				set_header_message('danger','Tambah Parameter','Gagal menambah parameter');
				redirect(base_url('tu/master/kriteria/subkriteriaadd').$link);
			}
		}else{
			$kriteria=$this->input->get('kriteria');
	        ...
	        $d['kriteria']=$kriteria;
	        $d['link']=$link;
	        $this->load->view('tu/master/kriteria/subkriteria/subkriteriaadd',$d);
	        $this->load->view('tema/footer');
		}
	}
    ...
    {		
		...
		{			
	        ...
	        $this->load->view('tu/master/kriteria/subkriteria/subkriteriaedit',$d);
	        $this->load->view('tema/footer');
		}
	}
