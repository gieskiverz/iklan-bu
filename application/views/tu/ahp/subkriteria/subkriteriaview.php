...
<div>
	<a href="<?=base_url('tu/ahp/subkriteria/add').$kriteria;?>" class="btn btn-primary btn-md">Tambah Parameter</a>
</div>
<p>&nbsp;</p>
	...
	<tbody>
		...
		if(!empty($data))
		{
			...
			<tr>
				...
				<td><?=$nilainama;?></td>
				<td>
					<a href="<?=base_url('tu/ahp/subkriteria/edit').'?id='.$id.$link;?>" class="btn btn-xs btn-info"><i class="fa fa-edit"></i></a>
							<a onclick="return confirm('Yakin ingin menghapus data ini?');" href="<?=base_url('tu/ahp/subkriteria/delete');?>?id=<?=$id.$link;?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
				</td>
			</tr>
			<?php
			}
