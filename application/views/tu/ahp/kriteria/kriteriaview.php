...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/ahp/kriteria/update",
		data:$(this).serialize(),
		error:function(){
...
<button type="button" class="btn btn-primary btn-md" data-toggle="modal" data-target="#modaldata">
  Data Kriteria Utama
</button>
<a href="<?=base_url('tu/ahp/kriteria/add');?>" class="btn btn-success btn-md">Tambah Kriteria Utama</a>
<div class="modal fade" id="modaldata" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-md" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
        <h4 class="modal-title">Data Kriteria Utama</h4>
      </div>
      <div class="modal-body">
      	<?php
		  echo validation_errors();
		  echo form_open(base_url('tu/ahp/kriteria/updatedata'),array('class'=>'form-horizontal'));
		  ?>		  		  
        <table class="table table-border">
        	...
        	<tbody>
        		...
        		if(!empty($arr))
        		{
					...
					<tr>
						<td>
						<input type="text" class="form-control" name="<?=$kk;?>" value="<?=$vv;?>"/>
						</td>
						<td>
							<a href="<?=base_url('tu/ahp/subkriteria');?>?kriteria=<?=$kk;?>" class="btn btn-xs btn-info">Parameter</a>
							<a onclick="return confirm('Yakin ingin menghapus data ini?');" href="<?=base_url('tu/ahp/kriteria/deletedata');?>?id=<?=$kk;?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
						</td>
					</tr>
					<?php
					}
